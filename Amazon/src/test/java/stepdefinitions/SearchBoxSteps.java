...
//    }
    ...
    @Then("user should see the valid {string} products appears1")
    public void user_should_see_the_valid_products_appears1(String expectedProduct) {
        verifyText(homePage.verifySearchedProduct, "\"" + expectedProduct + "\"", "Product name does not match");
    }
    ...
    @Then("user should see the valid product appears after search2")
    public void user_should_see_the_valid_product_appears_after_search2(DataTable dataTable) {
        List<List<String>> data = dataTable.asLists(String.class);
        String expectedProduct = data.get(1).get(0);
        verifyText(homePage.verifySearchedProduct, "\"" + expectedProduct + "\"", "Product name does not match");
        verifyText(homePage.verifySearchedProduct, "\"" + expectedProduct + "\"", "Product name does not match");
    }
        @Then("user should see the valid product appears after searchh")
        public void user_should_see_the_valid_product_appears_after_searchh(DataTable dataTable) {
            List<List<String>> data = dataTable.asLists(String.class);
            String expectedProduct=data.get(1).get(0);
            verifyText(homePage.verifySearchedProduct, "\"" + expectedProduct + "\"", "Product name does not match");
